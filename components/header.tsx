        ...
        <ul className="flex w:[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-slate-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            ...
            >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-slate-500 dark:hover:text-slate-300",
                  {
              ...
              >
                ...
                {link.name === activeSection && (
                  <motion.span
                    className="bg-slate-100 rounded-full absolute inset-0 -z-10 bg-opacity-80 backdrop-blur-[0.2rem] dark:bg-slate-800"
